...
外部适配器可以另外声明可选的 `launcher.path`，指向 `payload/` 下的 `.exe`、`.cmd` 或 `.bat` 文件。Desktop 只在用户点击“启动”时调用该文件，并将工作目录设为该文件所在目录；启动失败只提示用户，不会影响 Desktop。未声明时“启动”按钮显示为灰色不可用，仍可通过“打开目录”手动启动。该字段不支持参数、安装脚本或自动启动。
...
私钥不进入仓库、作者工具、PR 校验或版本发现机器人。维护者可以在本机离线签名，也可以在主分支 CI 成功后，通过要求人工批准的专用 GitHub Environment 完成签名；普通 CI 和未审核 PR 无法读取签名 Secret。
