...
GitHub 上的 `Publish signed registry` 工作流使用受保护的 `market-signing` Environment。主分支 CI 成功且当前签名失效时，维护者批准部署后，专用任务才可读取 `MARKET_SIGNING_PRIVATE_KEY`。签名提交会先在临时分支完成同一套 CI，再通过只包含签名的 PR 合并；Gitee 同步也会先验签，拒绝发布审批中的未签名索引。PR、版本发现机器人和普通 CI 均无法读取签名 Secret。
